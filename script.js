...
function menuitem12OnClick(Sender)
{
          ...
          {
              openURL("http://lightquick.co.uk/downloads/panzer-cpu-gauge-xwidget.html?Itemid=264");
          }
}
//=====================
//End function
...
function checkCpuUsage() {
   print("cpuAveText.text "+cpuAveText.text);

   var dd = cpuAveText.text;
...
function image99OnMouseDown(Sender,Button,Shift,X,Y)
{
md = true;
mdx = X;
mdy = Y;
...
function image99OnMouseMove(Sender,Shift,X,Y,Dx,Dy)
{
if(md)
{
...
function image99OnMouseUp(Sender,Button,Shift,X,Y)
{
md = false;
...
{
  print("cpuPointer.enabledAnimateChange "+cpuPointer.enabledAnimateChange);
  if (preferencessoundprefvalue.text != "disabled" ) {PlaySound(lock);};
  if (preferencesActionSwitchPrefValue.text === "continuous" ) {
...
function showWidgetPreferencesOnMouseDown(Sender,Button,Shift,X,Y)
{
